...
export class Crypt {
    ...
    static rootToken(): string {
        // let rootTokenAgeDays = (new Date().getTime() - Crypt._rootTokenAge.getTime()) / (1000 * 60 * 60 * 24);
        // let rootTokenAgeSeconds = (new Date().getTime() - Crypt._rootTokenAge.getTime()) / 1000;
        let rootTokenAgeMinutes = (new Date().getTime() - Crypt._rootTokenAge.getTime()) / 1000 / 60;
        if(rootTokenAgeMinutes > 60) {
            Crypt._rootToken = "";
        }
        if(Crypt._rootToken != "") return Crypt._rootToken;
        Crypt._rootToken = Crypt.createToken(this.rootUser(), "1d");
        return Crypt._rootToken;
    }
    ...
    static createToken(item: User | TokenUser, expiresIn: string): string {
        ...
        if (NoderedUtil.IsNullEmpty(Config.aes_secret)) {
            var b = true;
            throw new Error("Config missing aes_secret");
        }
        if (NoderedUtil.IsNullEmpty(Crypt.encryption_key)) Crypt.encryption_key = Config.aes_secret.substring(0, 32);
        const key = Crypt.encryption_key;
        if (NoderedUtil.IsNullEmpty(key)) throw new Error("Config missing aes_secret");
    ...
    static async verityToken(token: string, cli?: WebSocketServerClient, ignoreExpiration: boolean = false): Promise<TokenUser> {
        try {
            ...
            if(token == Crypt._rootToken) {
                return TokenUser.assign(Crypt.rootUser() as any);
            }
            if (NoderedUtil.IsNullEmpty(Config.aes_secret)) throw new Error("Config missing aes_secret");
            if (NoderedUtil.IsNullEmpty(Crypt.encryption_key)) Crypt.encryption_key = Config.aes_secret.substring(0, 32);
