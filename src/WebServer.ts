...
export class WebServer {
    ...
    static async configure(baseurl: string, parent: Span): Promise<http.Server> {
        ...
        try {
            ...
            //     var requestedfile = req.url.split("?")[0].split("#")[0].replace("/ui/", "");
            //     console.log("requestedfile", requestedfile);
            //     let file = path.join(WebServer.webapp_file_path, requestedfile);
            //     if(requestedfile.endsWith("/") && !fs.existsSync(file)) {
            //         requestedfile = requestedfile.substring(0, requestedfile.length - 1);
            //         file = path.join(WebServer.webapp_file_path, requestedfile);
            //     }
            //     if(requestedfile == "" || requestedfile == "/ui" || requestedfile.indexOf("auth/signinwin/main") > -1 ) {
            //         res.sendFile(path.join(WebServer.webapp_file_path, "index.html"));
