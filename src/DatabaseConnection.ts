...
export class DatabaseConnection extends events.EventEmitter {
    ...
    async connect(parent: Span = undefined): Promise<void> {
        ...
        var mongodburl = this.mongodburl;
        ...
        await this.doRegisterGlobalWatches(span);
        // }
    ...
    async query<T extends Base>(options: QueryOptions, span: Span): Promise<T[]> {
        // @ts-ignore
        let { query, projection, top, skip, orderby, collectionname, jwt, queryas, hint, explain, decrypt } = Object.assign({
            top: 100,
            skip: 0,
            decrypt: true
        }, options);
    ...
    async count(options: CountOptions, span: Span): Promise<number> {
        ...
        const ot_end = Logger.otel.startTimer();
        let result;
        if(Object.keys(query).length == 0 && user.HasRoleName("admins")) {
            result = await this.db.collection(collectionname).estimatedDocumentCount()
        } else {
            result = await this.db.collection(collectionname).countDocuments(_query);
        }
        // let result
    ...
    async distinct(options: DistinctOptions, span: Span): Promise<string[]> {
        ...
        const ot_end = Logger.otel.startTimer();
        // @ts-ignore
        let result = await this.db.collection(collectionname).distinct(field, _query, {explain: options.explain});
        span?.setAttribute("results", result);
        let ms = Logger.otel.endTimer(ot_end, DatabaseConnection.mongodb_count, DatabaseConnection.otel_label(collectionname, user, "count"));
        if (Config.log_database_queries && ms >= Config.log_database_queries_ms) {
    ...
    */
    async GetOne<T extends Base>(options: { query?: object, collectionname: string, orderby?: object, jwt?: string, decrypt?: boolean }, span: Span): Promise<T> {
        ...
        if(options.collectionname == null || options.collectionname == "") throw new Error("collectionname is mandatory");
        ...
        try {
            ...
            if (collectionname === "agents") {
                ...
                if(item._type == "agent") {
                    ...
                    if (!NoderedUtil.IsNullEmpty(agent.runas)) {
                        // SKIP !!!
                        // We do the check on docker/kubernetes driver level
                        ...
                        //         if(agent.stripeprice != null && agent.stripeprice != "") {
                        //         }
            ...
            if (collectionname === "users" && (item._type === "user" || item._type === "role")) {
                ...
                if (!NoderedUtil.IsNullEmpty(user2.customerid)) {
                    ...
                    if (customer == null) throw new Error("Access denied to customer with id " + user2.customerid + " when inserting " + user2.name);
    ...
    */
    ...
    async deleteIndex(collectionname: string, name: string, parent: Span) {
        if(collectionname == null || collectionname == "") throw new Error("collectionname is mandatory");
        if(name == null || name == "") throw new Error("indexname is mandatory");
    ...
    public async UpdateIndexTypes(parent: Span) {
        try {
            ...
            await Config.db.ParseTimeseries(parent);
        } catch (error) {
        }
    }
